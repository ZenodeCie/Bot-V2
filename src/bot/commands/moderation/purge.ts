...
export default {
  ...
  slash: [
    { name: "nombre", description: "Nombre de messages (1-500)", type: ApplicationCommandOptionType.Integer, required: true, minValue: 1, maxValue: 500 },
    {
      name: "type",
      description: "Filtre",
    ...
    { name: "utilisateur", description: "Cible (type user)", type: ApplicationCommandOptionType.User, required: false },
    { name: "texte", description: "Texte (type contains)", type: ApplicationCommandOptionType.String, required: false },
  ],
  slashArgs: (i: ChatInputCommandInteraction) => {
    const type = i.options.getString("type") ?? "all"
    const nombre = String(i.options.getInteger("nombre") ?? 0)
    if (type === "all") return [nombre]
    if (type === "user") return ["user", nombre, i.options.getUser("utilisateur")?.id ?? ""]
    if (type === "contains") return ["contains", nombre, i.options.getString("texte") ?? ""]
    return [type, nombre]
  },
