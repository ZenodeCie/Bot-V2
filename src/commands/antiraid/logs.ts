...
export default {
  ...
  async execute(client: Client, message: Message, args: string[]) {
    console.log(`Command logs used by ${message.author.tag} (${message.author.id}) in the guild ${message.guild?.name} (${message.guild?.id}${message.guild?.vanityURLCode ? ` / .gg/${message.guild?.vanityURLCode}` : ""})`)
